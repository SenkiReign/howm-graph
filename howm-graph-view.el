...
(defun howm-graph--make-html-content ()
  ...
  (concat
   ...
   ".btn { padding: 8px 16px; margin: 5px 5px 5px 0; background: #4a90e2; color: white; border: none; border-radius: 4px; cursor: pointer; }\n"
   ...
   "body.dark #sidebar { background: #161b22; border-left-color: #30363d; }\n"
   "body.dark h2 { color: #c9d1d9; }\n"
   "body.dark #details { background: #0d1117; border-left-color: #9b59b6; color: #c9d1d9; }\n"
   "body.dark #details h3 { color: #e6edf3; }\n"
   "body.dark #details em { border-top-color: #30363d; color: #8b949e; }\n"
   "</style>\n"
   "</head>\n"
   "<body>\n"
   "<div id=\"graph\"></div>\n"
   ...
   "  })\n"
   "  .attr('fill', d => d.type === 'keyword' ? '#ff7f0e' : '#9b59b6')\n"
   "  .attr('stroke', '#f0f0f0')\n"
   ...
   "    const file = d.file || '';\n"
   "    document.getElementById('details').innerHTML = '<h3>' + d.heading + '</h3><p>' + content + '</p>' + (file ? '<em>' + file + '</em>' : '');\n"
   "  });\n"
   ...
   "  labels.attr('fill', darkMode ? '#eee' : '#000');\n"
   "};\n"
