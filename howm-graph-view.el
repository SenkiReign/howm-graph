...
(defun howm-graph--make-html-content ()
  ...
  (concat
   ...
   "#sidebar { width: 300px; min-width: 200px; max-width: 600px; border-left: 1px solid #ddd; padding: 20px; overflow-y: auto; background: white; position: relative; }\n"
   ...
   "#details { padding: 15px; background: #f9f9f9; border-radius: 8px; border-left: 4px solid #9b59b6; line-height: 1.8; }\n"
   ...
   "body.dark #sidebar { background: #000; border-left-color: #00ff00; }\n"
   "body.dark h2 { color: #00ff00; }\n"
   "body.dark #details { background: #000; border-left-color: #00ff00; color: #00ff00; }\n"
   "body.dark #details h3 { color: #00ff00; }\n"
   "body.dark #details em { border-top-color: #00ff00; color: #00ff00; }\n"
   "body.dark #search { background: #000; color: #00ff00; border-color: #00ff00; }\n"
   "body.dark #search::placeholder { color: #00ff0088; }\n"
   "body.dark #search-results { color: #00ff00; }\n"
   "body.dark #stats { color: #00ff00; }\n"
   "body.dark .btn { background: #003300; color: #00ff00; border: 1px solid #00ff00; }\n"
   "body.dark .btn:hover { background: #005500; }\n"
   "body.dark .btn.secondary { background: #002200; }\n"
   "body.dark .btn.active { background: #004400; }\n"
   "</style>\n"
   "</head>\n"
   "<body>\n"
   "<div id=\"graph\"></div>\n"
   ...
   "</div>\n"
   "<h2>Search</h2>\n"
   "<input type=\"text\" id=\"search\" placeholder=\"Search nodes...\" style=\"width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-bottom: 10px;\">\n"
   ...
   "  .attr('stroke', '#000').attr('stroke-width', 1.5).attr('stroke-opacity', 0.6);\n"
   ...
   "  })\n"
   "  .attr('fill', d => {\n"
   "    if (d.type === 'keyword') return '#000';\n"
   "    if (d.degree === 0) return '#666';\n"
   "    return '#333';\n"
   "  })\n"
   "  .attr('stroke', '#fff')\n"
   ...
   "  labels.attr('fill', darkMode ? '#00ff00' : '#000');\n"
   "  node.attr('fill', d => {\n"
   "    if (darkMode) {\n"
   "      if (d.type === 'keyword') return '#00ff00';\n"
   "      if (d.degree === 0) return '#008800';\n"
   "      return '#00cc00';\n"
   "    } else {\n"
   "      if (d.type === 'keyword') return '#000';\n"
   "      if (d.degree === 0) return '#666';\n"
   ...
   "    }\n"
   "  });\n"
   "  node.attr('stroke', darkMode ? '#003300' : '#fff');\n"
   "  link.attr('stroke', darkMode ? '#00ff00' : '#000');\n"
   "};\n"
